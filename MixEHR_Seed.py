...
class MixEHR_Seed(nn.Module):
    ...
    def save_parameters(self, epoch):
        torch.save(self.exp_m, "/lustre04/scratch/zyq1213/round_all_exp_m_%s.pt" % (epoch))
        for i, modality in enumerate(self.modalities):
            torch.save(self.exp_n[i], "/lustre04/scratch/zyq1213/round_all_exp_n_%s_%s.pt" % (modality, epoch))
        torch.save(self.exp_s, "/lustre04/scratch/zyq1213/round_all_exp_s_%s.pt" % (epoch))
        torch.save(self.pi, "/lustre04/scratch/zyq1213/round_all_pi_%s.pt" % (epoch))
